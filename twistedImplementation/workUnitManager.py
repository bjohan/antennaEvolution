...
class WorkUnitManager:
    ...
    def __str__(self):
        ...
        a += "Summary of computer clients\n"
        for c in self.computers.clients:
            a += "\t Computer " + str(c.computerId) + " has " \
                 + str(c.workUnitsAtClient) + "work units\n"
        a += "Summary of work generator clients\n"
        for c in self.generators.clients:
            a += "\t Work generator " + str(c.workGeneratorId) + "\n"
        return a
